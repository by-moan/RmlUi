...
#define ___EMAP_INCLUDE
#include "../../Include/RmlUi/Core/ElementMap.h"
#undef ___EMAP_INCLUDE
namespace Rml {
...
namespace _a_E_D {
	...
	ElementPtr_Handler::ElementPtr_Handler(ElementPtr element) : ownedElement(std::move(element)) {}
...
Element* ElementDocument::__createElement(const String& name){
	_a_E_D::ElementPtr_Handler* elm = new _a_E_D::ElementPtr_Handler(CreateElement(name));
	auto eGet = elm->ownedElement.get();
	// std::cout << eGet << " createElement()\n";
	_a_E_D::elementMap.emplace(eGet, elm);
	return eGet;
}
	...
	if (event == EventId::Keydown)
	{
			...
			key_identifier == Input::KI_DOWN)
		{
			...
			auto GetNearestFocusable = [this](Element* focus_node) -> Element* {
				while (focus_node)
				{
					if (_a_E_D::CanFocusElement(focus_node) == _a_E_D::CanFocus::Yes)
						break;
...
Element* ElementDocument::FindNextTabElement(Element* current_element, bool forward)
{
	...
	if (current_element != document && _a_E_D::CanFocusElement(document) == _a_E_D::CanFocus::Yes)
		return document;
...
{
	_a_E_D::CanFocus can_focus = _a_E_D::CanFocusElement(element);
	if (can_focus == _a_E_D::CanFocus::Yes)
		return element;
	else if (can_focus == _a_E_D::CanFocus::NoAndNoChildren)
		return nullptr;
...
Element* ElementDocument::FindNextNavigationElement(Element* current_element, NavigationSearchDirection direction, const Property& property)
{
	...
	auto GetNearestScrollContainer = [this](Element* element) -> Element* {
		...
		{
			if (_a_E_D::IsScrollContainer(element))
				return element;
		}
		return this;
	};
	...
	_a_E_D::SearchNavigationResult best_result;
	_a_E_D::SearchNavigationTarget(best_result, start_element, direction, bounding_box, current_element);
